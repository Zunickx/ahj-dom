export default class PlayingArea { // определяем класс PlayingArea, который дальше будем экспортировать по умолчанию
    constructor(element) {
    ...
    randomMovingGoblin() {
      ...
      for(const playingItem of playingItemsCollection) { // Если у элемента есть первый дочерний элемент, то он удаляется с помощью метода remove
